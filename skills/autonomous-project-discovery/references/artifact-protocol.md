...
The script pre-arms `EVENTS.FROZEN` before any possible event-stream write and removes it only after write, durability, close, identity, prefix, delta, JSON, and newline postconditions pass. If the marker exists or an append fails, stop all later event appends. Never delete the marker, retry the same stream, or repair old event bytes.
