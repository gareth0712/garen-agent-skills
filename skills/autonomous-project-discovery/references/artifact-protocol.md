...
Before initializing the real stream, run these exact prefix/delta/parse checks on an isolated scratch file inside the run root, record only the bounded result in bootstrap evidence, and remove the scratch file. If the host cannot execute and verify this contract, block bootstrap with a `capability` gate instead of weakening the audit.
