...
| `append_event` | Use an exposed append-capable filesystem API or a small verified script/command that opens `EVENTS.jsonl` in append mode and performs one byte write. | Use an exposed append-capable filesystem API or a small verified script/command that opens `EVENTS.jsonl` in append mode and performs one byte write. | Before append, verify the existing prefix is complete JSONL and record its byte length/SHA-256. Submit UTF-8 JSON plus exactly one newline in one append write, close/flush, then prove unchanged prefix and exact parsed byte delta. Verify the same contract on a scratch file first. Never edit, patch, truncate, or rewrite an existing stream; block if any invariant is unavailable or false. |
