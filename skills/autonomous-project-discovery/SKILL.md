...
When preflight passes, dispatch one fresh worker with the packet verbatim, applicable instruction paths, capability/model record, ten-line return contract, portable artifact paths, and the orchestrator-recorded lexical plus physical/canonical anchors. The worker repeats normalization, no-follow component inspection, and physical containment before its first write, and blocks on mismatch; this is defense in depth, not independent acceptance. After work, it inventories the assigned root and checks scoped out-of-root changes. Never rely on a relative path when worktrees, nested agents, or differing patch/shell bases are possible. Wait using only the host-supported mechanism.
