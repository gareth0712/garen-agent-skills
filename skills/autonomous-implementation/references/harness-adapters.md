...
A dispatched worker receives the Task Contract verbatim, accepted Discovery/Planning revisions, applicable instructions, exact product/artifact writes, exact side effects, requested/effective model contract, verification/fallback/UAT, progress boundary, physical anchors, and at-most-ten-line return contract. The worker repeats containment checks, writes its progress marker, then changes only owned paths. The orchestrator rechecks containment, dirty paths, diffs, commands, and representative behavior before acceptance.
